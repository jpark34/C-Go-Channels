...
typedef struct {
    ...
    void* data;
    pthread_mutex_t mutex;      //mutex for the channels
    int channel_closed;         //channel open = 0, channel closed = 1
    int send_wait;              //holds number of waiting send threads
    pthread_cond_t send_cond;   //condition that when signaled will let a thread add to the buffer
    int receive_wait;           //holds number of waiting receive threads
    pthread_cond_t receive_cond; //condition that when signaled will let a receive thread go
